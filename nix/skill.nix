...
# every agent client on the machine. A standalone user will get the identical
# bytes from `perch skill install` — step 4 of the standard's rollout, not a
...
runCommand "perch-skill"
  ...
  ''
    ...
    # not knowing perch exists. That is the failure worth failing a build over.
    ...
    printf '%s\n' "$front" | grep -q '^name: perch$' \
      || { echo "ai/SKILL.md's frontmatter has no 'name: perch' line" >&2; exit 1; }
