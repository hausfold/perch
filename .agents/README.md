...
belong to the workshop. When perch grows one of its own it goes in
`.agents/skills/<name>/SKILL.md`, symlinked into `.claude/skills/<name>/` and
`.opencode/skills/`, never copied.

## Caveats

- **Perch is a macOS app; a Linux cloud container can't build it.** `xcodebuild`
  is macOS-only, and so is every signing step. The bootstrap is there so a cloud
  session can still resolve and re-lock the flake — not so it can produce a
  `Perch.app`.
