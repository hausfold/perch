...
# container, CI) boot a bare Linux box with no Nix. Perch ships to the family as
# a flake — `flake.nix`, `nix/release.nix`, `nix/package.nix` — so without Nix
# the session can't regenerate `flake.lock` or evaluate any of it. This installs
# Determinate Nix once, puts it on PATH for the rest of the session, and points
# Nix at the agent proxy's CA.
#
# Wired up as:
