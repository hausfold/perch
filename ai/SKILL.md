---
name: perch
description: Put files on the Mac's notch shelf — the tray that lives in the screen's notch and holds things you're about to drag somewhere. Use when the user says "put this in my shelf", "add this to perch", "shelf this", "hold onto this file", "stick that on the notch", "park these screenshots somewhere I can drag them", or hands you a file and asks you to keep it handy. Also covers checking whether Perch is running and why an add was refused.
---
...
| everything, exhaustively | `perch add --help` |
...
`{name, path}` (plus `reason` on the last two). The paths are your own
arguments echoed back — they are never sent to Perch.
...
| 2 | Perch turned items away | report it; don't retry the same batch |
| 3 | no Perch answered in time | it may be slow to launch — retry with `--wait 30` |
...
- **One Perch owns the notch.** If the user has a dev build running alongside
  the installed app, adds can land on the wrong one.
