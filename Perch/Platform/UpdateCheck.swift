...
// MARK: - InstallKind, live
//
// The enum itself is `PerchDiagnostics/InstallKind.swift` — shared with the
// `perch` tool, which needs the same answer with no app running. This half is
// the app's own reading of the receipts, and it stays here because it reaches
// for `RiceFiles`, which the tool does not compile.

extension InstallKind {
