...
/// intents. Deliberately thin — id, name, kind, nothing more — because it's a
/// live view onto `ShelfStore.items`, not a copy of its own; anything that
/// needs the staged bytes re-reads the real `ShelfItem` from the store by
/// `id`.
