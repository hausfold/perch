...
- **it lives in Finder's right-click menu** — **Add to Perch Shelf** sends selected files or folders straight to the shelf from Finder's Quick Actions menu. macOS turns it on for you; if it ever goes missing, it lives in System Settings ▸ General ▸ Login Items & Extensions ▸ Extensions ▸ ⓘ next to **System Services** — under that row, not under Perch. Perch must be running so it can reserve shelf space before Finder copies anything.
