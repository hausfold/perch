...
- Every invariant of a drag holds for a script: originals are only read,
  admission is decided before staging, no original path reaches the container
  or a log, and a tile appears only after a complete copy is adopted.
